...
export default function Home() {
  ...
  const handleCreateChad = async () => {
    ...
    try {
      ...
      if (!tRes.ok) throw new Error(tData?.error || 'transform failed');

      if (typeof tData?.baseImagePath !== 'string' || !Array.isArray(tData?.layers)) {
        throw new Error('invalid transform payload (missing baseImagePath/layers)');
      }
      ...
      const ipfsRes = await fetch('/api/upload-to-ipfs', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          baseImagePath: tData.baseImagePath,
          layers: tData.layers,
          name: `Farchad #${fid}`,
      ...
      if (!ipfsData?.metadataUri) throw new Error('missing metadataUri from ipfs');
